'''
A program to play video every n second to give a break to the user
'''
import random
import sys
import time
...
if int(selected_singer) == 1:
    while True:
        video = random.choice(list(michael_jackson.keys()))
        print("Michael Jackson video: ", video, 'is playing, enjoy!')
        time.sleep(3)
        chrome_controller = web.get('chrome')
        chrome_controller.open(michael_jackson[video])
        time.sleep(int(wait_time)*60)
elif int(selected_singer) == 2:
    while True:
